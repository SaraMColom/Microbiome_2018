...
```{r}
NMS_points <- NMS$points

# Add sample info to points data
...
Merge root data & NMS results

```{r}
NMS_points <- merge(NMS_points, RootData[c("Sample_ID", "PC1", "PC2", "PC3", "PC4")])
```
...
```{r}
NMS_points <- NMS_points %>% 
  mutate(PC2_cat = case_when(
    PC2 < -2 ~ "Low",
    PC2 >= -2 & PC2 < 1.5 ~ "Medium",
    PC2 >= 1.5 ~ "High"
  )) %>% 
  mutate(PC2_cat = factor(PC2_cat, c("Low", "Medium", "High")))

ggplot(NMS_points, aes(MDS1, MDS2, group = PC2_cat, color = PC2_cat)) +
  geom_point(size = 2) +
  theme_classic()
```
